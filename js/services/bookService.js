...
function deleteReview(book, review) {
  var idxRevToDelete = book.reviews.findIndex(rev => rev === review);
  book.reviews.splice(idxRevToDelete, 1);
  utilService.saveToStorage('books', booksDB);
...
function getBooks() {
  var booksFromStorage = utilService.loadFromStorage('books');
  if (booksFromStorage) booksDB = booksFromStorage;
  return Promise.resolve(booksDB);
}

function getBookById(bookId) {
  var searchBook = booksDB.find(book => {
    return bookId === book.id
  });
  return Promise.resolve(searchBook);
}

function searchBook(bookNameToSearch) {
  console.log('server');
  
  return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${bookNameToSearch}`)
  .then(res => res.data)
}

function addGoogleBook({id, volumeInfo, saleInfo}) {
  console.log('service add google book');
  var bookExist = _findIdxById(id);
  if (bookExist) return Promise.reject(bookExist);

  var newBook = {}
    newBook.id = id,
    newBook.title = (volumeInfo.title) ? volumeInfo.title : 'No title',
    newBook.subtitle = (volumeInfo.subtitle) ? volumeInfo.subtitle : 'No subtitle',
    newBook.authors = (volumeInfo.authors) ? volumeInfo.authors : ['No authors'],
    newBook.publishedDate = (volumeInfo.publishedDate) ? volumeInfo.publishedDate : 0,
    newBook.description = (volumeInfo.description) ? volumeInfo.description : 'No description',
    newBook.pageCount = (volumeInfo.pageCount) ? volumeInfo.pageCount : 0,
    newBook.categories = (volumeInfo.categories) ? volumeInfo.categories : ['No categories'],
    newBook.thumbnail = (volumeInfo.imageLinks.thumbnail) ? volumeInfo.imageLinks.thumbnail : 'http://coding-academy.org/books-photos/2.jpg',
    newBook.language = (volumeInfo.language) ? volumeInfo.language : 'en',

    newBook.listPrice = {amount: 0, currencyCode: null, isOnSale: false }
    if (saleInfo.listPrice) {
      newBook.listPrice.amount = (saleInfo.listPrice.amount) ? saleInfo.listPrice.amount : 0,
      newBook.listPrice.currencyCode = (saleInfo.listPrice.currencyCode) ? saleInfo.listPrice.currencyCode : 'USD',
      newBook.listPrice.isOnSale = (saleInfo.saleability === 'FOR_SALE') ? true : false
    }

  booksDB.unshift(newBook);
  utilService.saveToStorage('books', booksDB);
